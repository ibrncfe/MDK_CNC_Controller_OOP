...
///////////////Functions of Stepper Motor/////////////

StepperMotor::StepperMotor() {}

StepperMotor::~StepperMotor() {}

void StepperMotor::setRequiredPosition(unsigned int required_pos) {}

void StepperMotor::setCurrentPosition(unsigned int current_position) {}

void StepperMotor::setSettingPR(unsigned int pulse_revolution) {}
...
void StepperMotor::run(ROT_DIRECTION direction, unsigned angle, unsigned speed)
{
	running = RUNNING;
	TIM_HandleTypeDef ht = this->HWSetting->htim;
	//CHANGE SETTING TIMER 5 CHANNEL 2
	__HAL_TIM_SET_PRESCALER	(&ht , 1);
	__HAL_TIM_SET_AUTORELOAD(&ht, speed);
	__HAL_TIM_SET_COMPARE(&ht, this->HWSetting->TIMCHANNEL, speed/2);
	...
	if (direction==RIGHT)
	{
		HAL_GPIO_WritePin(this->HWSetting->GPIO_DIR, this->HWSetting->GPIO_PinDIR, GPIO_PIN_RESET);
	}
	else
	{
...
void ServoMotor::RunServo(int direction, unsigned angle, unsigned speed)
{


}

void ServoMotor::checkStepper(StepperMotor& S) {}
